...
void Assert::assertTrue(const char* message, bool condition) {
	if (!condition) {
		Log::wtf(LOG_TAG, "%s (%d)", message, condition);
	}
	assert(condition);
}
...
void Assert::assertNull(const char* message, void* ptr) {
	if (ptr != NULL) {
		Log::wtf(LOG_TAG, "%s (0x%x)", message, ptr);
	}
	assert(ptr == NULL);
}
...
void Assert::assertEquals(const char* message, bool expected, bool actual) {
	if (expected != actual) {
		Log::wtf(LOG_TAG, "%s (%d, %d)", message, expected, actual);
	}
...
void Assert::assertEquals(const char* message, int64_t expected, int64_t actual) {
	if (expected != actual) {
		Log::wtf(LOG_TAG, "%s (%lld, %lld)", message, expected, actual);
	}
	assert(expected == actual);
}
...
void Assert::assertEquals(const char* message, void* expected, void* actual) {
	if (expected != actual) {
		Log::wtf(LOG_TAG, "%s (0x%x, 0x%x)", message, expected, actual);
	}
	assert(expected == actual);
}
